...
class _BarChartRace(CommonChart):
    ...
    def offset_image(self, location, length, name, ax):
        """
        ...
        if self.orientation == 'h':
            ab = AnnotationBbox(im, (length, location,), xybox=(0, 0.), frameon=False, xycoords='data',
            ...
            ab = AnnotationBbox(im, (location, length,), xybox=(0., 0), frameon=False, xycoords='data',
                                boxcoords='offset points', pad=0)
            if self.tick_label_mode == 'mixed':
                # load text as TextArea object
                label_text = TextArea(name)
